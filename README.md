...
1) 역할 중간 점검
    - 김윤호: 배경과 게임 캐릭터 사이의 물리적인 오류 해결방법 구상중(추후 알고리즘을 통해 기존의 방식과는 다른 방법으로 물리적 오류 해결 진행 계획, 몬스터는 성공)
    - 이하연: 게임 시작 화면 구현 완료 및 창(샌드위치, 커피, 연구실 컴퓨터) GUI 구현 완료
    - 최예린: A* 알고리즘을 사용하여 캐릭터 AI 이동 
    - 노수연: 상호작용 창 구현중에 있음(창 뜨는것 완료, 창 디자인만 입히면 안료)
    
2) 중간 보고서<br>
    - 각자 맡은 부분에 대한 역할 상세 서술한 파일(한글파일) 및 현재까지 구현한 코드 파일(한글파일) 작성하여 4월 20일 (목요일) 오후 8시까지 '최예린'에게 제출.
